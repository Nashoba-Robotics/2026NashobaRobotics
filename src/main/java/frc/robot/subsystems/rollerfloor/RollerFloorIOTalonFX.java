...
public class RollerFloorIOTalonFX implements RollerFloorIO {
  ...
  public RollerFloorIOTalonFX() {
    ...
    follower.setControl(new Follower(Constants.RollerFloor.LEADER_MOTOR_ID, MotorAlignmentValue.Aligned));
    ...
    BaseStatusSignal.setUpdateFrequencyForAll(
        1 / Constants.loopTime, leaderTemp, leaderVelocity, leaderAppliedVolts, leaderStatorCurrent, leaderSupplyCurrent, followerTemp, followerVelocity, followerAppliedVolts, followerStatorCurrent, followerSupplyCurrent);

    leader.optimizeBusUtilization();

    PhoenixUtil.registerSignals(false, leaderTemp, leaderVelocity, leaderAppliedVolts, leaderStatorCurrent, leaderSupplyCurrent, followerTemp, followerVelocity, followerAppliedVolts, followerStatorCurrent, followerSupplyCurrent);
  }

  @Override
  public void updateInputs(RollerFloorIOInputs inputs) {
    BaseStatusSignal.refreshAll(leaderTemp, leaderVelocity, leaderAppliedVolts, leaderStatorCurrent, leaderSupplyCurrent, followerTemp, followerVelocity, followerAppliedVolts, followerStatorCurrent, followerSupplyCurrent);

    inputs.leaderConnected =
        BaseStatusSignal.isAllGood(leaderTemp, leaderVelocity, leaderAppliedVolts, leaderStatorCurrent, leaderSupplyCurrent);
    ...
    inputs.followerConnected =
        BaseStatusSignal.isAllGood(followerTemp, followerVelocity, followerAppliedVolts, followerStatorCurrent, followerSupplyCurrent);
    inputs.followerTempCelsius = followerTemp.getValueAsDouble();
    inputs.followerVelocityRadsPerSec = Units.rotationsToRadians(followerVelocity.getValueAsDouble());
